      ...
      <section className="section-shell pt-6">
        <div className="columns-1 gap-5 sm:columns-2 lg:columns-3">
          {images.map((image, index) => (
            <figure key={`${image.src}-${index}`} className="mb-5 break-inside-avoid overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.06]">
              <img src={image.src} alt={image.alt} className="h-auto w-full object-cover transition duration-700 hover:scale-105" />
              <figcaption className="px-5 py-4 text-sm font-bold uppercase tracking-[0.18em] text-slate-300">
                {image.label}
              </figcaption>
            </figure>
          ))}
